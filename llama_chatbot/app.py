        ...
        with st.spinner("Думаю..."):
            try:
                if "llm" in st.session_state:
                    ...
                    # Выводим информацию о теме красиво и понятно
                    st.info(f"Определённая тема: {topic_result['topic_name']} "
                            f"(код {topic_result['topic_code']})\n"
                            f"Причина: {topic_result['reasoning']}")

                    # Используем код темы для RAG-ветки
                    if topic_result["topic_code"] != 9:
                        # Если тема определена, запускаем RAG-ветку:
                        # ищем 5 релевантных чанков и генерируем ответ на их основе
                        ...
                        try:
                            ...
                            rag_message = f"Ты должен отвечать ТОЛЬКО по данным тебе источникам (не придумывая ничего от себя) в формате JSON со следующей структурой: answer: \"твой ответ на вопрос\", \"sources\": [{first}, {second}, {third}, {fourth}, {fifth}], \"confidence\": число от 0 до 1\n"+f"{user_input} - вопрос пользователя\n {first} - первый источник\n {second} - второй источник\n {third} - третий источник\n {fourth} - четвёртый источник\n {fifth} - пятый источник\n , note: \"примечание об источниках, если необходимо\""
                            ...
                            if json_match:
                                try:
                                    ...
                                    # Сохраняем в архив полный JSON с данными (ответ, источники, уровень уверенности и примечание)
                        ...
                        # Если тема не определена, используем обычный режим LLM
                        human_msg = HumanMessage(content=user_input)
                        current_messages.append(human_msg)
                        save_message(st.session_state.current_chat_id, "user", user_input, temperature)
                        response = st.session_state.llm.invoke(current_messages)
